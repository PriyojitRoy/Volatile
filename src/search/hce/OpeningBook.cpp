...
namespace ChessCore {
    ...
    uint64_t OpeningBook::computePolyglotHash(const Board& board) {
        ...
        for (int sq = 0; sq < 64; sq++) {
            ...
            if (kind != -1) {
                hash ^= PolyglotRandoms[64 * kind + sq];
            }
        }
    
        if (board.castlingRights & CastlingWhiteKingSide)  hash ^= PolyglotRandoms[768];
        if (board.castlingRights & CastlingWhiteQueenSide) hash ^= PolyglotRandoms[769];
        if (board.castlingRights & CastlingBlackKingSide)  hash ^= PolyglotRandoms[770];
        if (board.castlingRights & CastlingBlackQueenSide) hash ^= PolyglotRandoms[771];
...
const uint64_t OpeningBook::PolyglotRandoms[781] = {
